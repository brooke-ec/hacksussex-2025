...
class BLEScanner:
    ...
    def _irq(self, event, data):
        if event == 5:  # Event value for _IRQ_SCAN_RESULT
            ...
            if rssi > -80 and namespace_id == self._target_namespace_id:
...
def general_ble_scan():
    target_namespace_id = 'mpy-uart'  # Replace with your target Namespace ID
    ble = bluetooth.BLE()
    scanner = BLEScanner(ble, target_namespace_id)

    print("Scanning for BLE advertisers with RSSI > -80 and matching target Namespace ID...")
    scanner.start_scan(duration_ms=15000)  # Scan for 15 seconds
    time.sleep(15)
    scanner.stop_scan()
